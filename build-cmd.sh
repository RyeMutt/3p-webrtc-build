...
case "$AUTOBUILD_PLATFORM" in
    darwin*)
        mkdir "x86_64"
        pushd "x86_64"
            tar -xjf "$stage"/_package/webrtc.macos_x86_64.tar.bz2 --strip-components=1
        popd

        mkdir "arm64"
        pushd "arm64"
            tar -xjf "$stage"/_package/webrtc.macos_arm64.tar.bz2 --strip-components=1
        popd
    ...
    ;;
    *)
        tar -xjf "$stage"/_package/webrtc."$build_type".tar.bz2 --strip-components=1
