...
case "$AUTOBUILD_PLATFORM" in
    darwin*)
        mkdir "x86_64"
        pushd "x86_64"
            tar -xjf  ~/_packages/webrtc.macos_x86_64/webrtc.tar.bz2 --strip-components=1
        popd

        mkdir "arm64"
        pushd "arm64"
            tar -xjf ~/_packages/webrtc.macos_arm64/webrtc.tar.bz2 --strip-components=1
        popd
    ...
    ;;
    *)
        tar -xjf ~/_packages/webrtc."$build_type"/webrtc.tar.bz2 --strip-components=1
